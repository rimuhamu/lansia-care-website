    ...
    <footer className="bg-foreground text-background py-8 lg:py-12 px-4 sm:px-6 lg:px-8">
      <div className="container mx-auto max-w-6xl">
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-6 lg:mb-8">
          ...
          {/* Quick Links */}
          <div>
            <h4 className="font-bold text-sm sm:text-base mb-3">{footerContent.programs.title}</h4>
            <ul className="space-y-1 text-background/80">
              {footerContent.programs.links.map((link, index) => (
                <li key={index}>
                  <a href="#" className="hover:text-background transition-colors text-xs sm:text-sm">
                    {link}
                  </a>
                </li>
              ))}
          ...
          {/* Contact & Social */}
          <div>
            <h4 className="font-bold text-sm sm:text-base mb-3">{footerContent.social.title}</h4>
            <div className="space-y-3">
              ...
              <div>
                <h5 className="font-semibold mb-1 text-xs sm:text-sm">{contactContent.info.hours.title}</h5>
        ...
        <div className="border-t border-background/20 pt-4 lg:pt-6">
          <div className="flex flex-col md:flex-row justify-between items-center gap-3">
            <p className="text-background/60 text-xs">{footerContent.legal.copyright}</p>
            <div className="flex gap-4 text-xs text-background/60">
              {footerContent.legal.links.map((link, index) => (
                <a key={index} href="#" className="hover:text-background transition-colors">
                  {link}
                </a>
              ))}
            </div>
